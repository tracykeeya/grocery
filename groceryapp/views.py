...
#we are creating avariable pointing it to all the objects in it#
    ...
    return render(request,'adminapp/home.html',{'products':products,'product_filters':product_filters})
...
def manage_credit_sale(request, credit_sale_id=None):
    ...
    return render(request, 'adminapp/manage_credit_sale.html', {'form': form, 'credit_sale': credit_sale})
...
def issue_item(request, pk):
    ...
    return render(request, 'adminapp/issue_item.html', {'sales_form': sales_form})

       
@login_required
def receipt(request):
    sales = Sale.objects.all().order_by('-id')
    return render(request,'adminapp/receipt.html',{'sales':sales})
@login_required
def receipt_detail(request, receipt_id):
    receipt = Sale.objects.get(id=receipt_id)
    return render(request, 'adminapp/receipt_detail.html', {'receipt':receipt})



@login_required
def loginout(request):
    return render(request, 'adminapp/logout.html')


@login_required
def daily_sales(request):
    sales = Sale.objects.all().order_by('-id')  # You can filter this to only show today's sales
    return render(request, 'adminapp/daily_sales.html', {'sales': sales})
...
def edit_sale(request, sale_id):
    ...
    if request.method == 'POST':
        ...
        if sale_form.is_valid():
            ...
            return render(request, 'adminapp/edit_sale.html', {'sale_form': sale_form, 'sale': sale})
    else:
...
def add_stock(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Form contains invalid fields.')
    else:
        form = StocksForm()

    return render(request, 'adminapp/add_stock.html', {'form': form})
...
def add_item(request, pk):
    ...
    return render(request, 'adminapp/add_item.html', {'form': form})
